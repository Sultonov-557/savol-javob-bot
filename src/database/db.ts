...
export async function getUser(ID: string) {
  return await userRepo.findOne({ where: { ID }, relations: ["results", "results.question"] });
}

export async function isAnswered(userID: string, questionID: number) {
  const user = await userRepo.exist({ where: { ID: userID, results: { question: { ID: questionID } } }, relations: ["results"] });
  return user;
}
...
export async function getNextQuestion(user: User, theme: string, currentQuestionID: number) {
  const questions = await questionRepo.find({ where: { theme: theme } });
  const results = await resultRepo.find({ where: { user: { ID: user.ID } }, relations: ["user", "question"] });
...
export async function getResultsByTheme(userID: string, theme: string) {
  return await resultRepo.find({ where: { question: { theme }, user: { ID: userID } }, relations: ["question", "user"] });
}
...
export async function newResult(arg: { user: User; question: Question; answerTime: Date; correct: boolean }) {
